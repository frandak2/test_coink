...
To move beyond notebook prototyping, all reusable code should go into the `test_coink/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `test_coink` folder and use the modules inside your notebooks :
...
```python
from test_coink.utils.paths import data_dir
data_dir()
```
